...
import { IFRAME_URL_END_BIGTRAILER, IFRAME_URL_START} from "../../utils/constants";

const BigTrailer = React.memo(({ selectedMovie }) => {
  const { movieId, title, gemini } = selectedMovie;
  const dispatch = useDispatch();

  const geminiMovies = useSelector((store) =>
    gemini ? store.gemini.geminiMovieResults : null
  );
  const geminiMoviesArray = geminiMovies && Object.values(geminiMovies.flat());

  const listName = useMemo(
    () => (!gemini ? toCamelCase(title) : null),
    [gemini, title]
  );
  const movies = useSelector((store) =>
    !gemini ? store.movies?.[listName] : null
  );

  const movie = gemini
    ? geminiMoviesArray?.find((movie) => movie.id === movieId)
      ...
      {!trailerKey ? (
        ...
        <>
          <iframe
            ...
            src={IFRAME_URL_START + `${trailerKey}` + IFRAME_URL_END_BIGTRAILER}
          ...
          <div className="bg-black p-6 text-white space-y-4 shadow-lg">
            ...
            {(vote_average || vote_count) && (
              <div className="flex items-center space-x-4 mt-2">
                {vote_average && (
                  <span className="px-3 py-1 rounded-lg text-sm md:text-lg font-semibold">
            ...
            {adult && (
              <span className="bg-red-700 py-1 rounded-md text-sm md:text-lg font-semibold">
                18+
              </span>
            )}
          </div>
