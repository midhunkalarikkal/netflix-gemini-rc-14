  ...
  return (
    <div className="absolute bg-gradient-to-r from-black w-screen aspect-video">
      <div className="absolute bottom-1 md:bottom-16 lg:bottom-36 left-2 md:left-6 lg:left-8 p-1 md:p-4 text-white">
        {movieMetaData?.logo ? (
          <img className="w-32 md:w-48 lg:w-72" src={IMG_CDN + movieMetaData?.logo} alt="movie_title"/>
        ) : (
          <h1 className="font-bold text-xl md:text-3xl lg:text-6xl">{original_title}</h1>
        )}
        <p className="md:text-sm lg:text-md md:w-6/12 lg:w-4/12 py-1 hidden md:block">{overview}</p>
        <p className="text-xs sm:text-sm w-7/12 py-1 md:hidden">{overview.slice(0,60)+"..."}</p>
